...
export const TerminalContent: React.FC = () => {
  ...
  return (
    <main className="flex-1 overflow-y-auto custom-scrollbar p-8 lg:p-12 space-y-20 scroll-smooth bg-terminal-body">
      {/* Header / Intro */}
      ...
      <section className="max-w-4xl" id="whoami">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h2 className="text-2xl font-bold text-white uppercase tracking-wider">Acerca de Mi</h2>
          <div className="h-px bg-white/10 flex-1"></div>
        </div>
        <div className="bg-[#1a1a1a]/40 border border-white/5 p-8 rounded-xl backdrop-blur-sm">
      ...
      <section className="max-w-4xl" id="history">
        <div className="flex items-center gap-4 mb-8">
          <span className="text-primary font-bold text-2xl">#</span>
          <h2 className="text-2xl font-bold text-white uppercase tracking-wider">Work History</h2>
          <div className="h-px bg-white/10 flex-1"></div>
        </div>
      ...
      <section className="max-w-5xl" id="skills">
        <div className="flex items-center gap-4 mb-8">
          <span className="text-primary font-bold text-2xl">#</span>
          <h2 className="text-2xl font-bold text-white uppercase tracking-wider">Skills --list</h2>
          <div className="h-px bg-white/10 flex-1"></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono">
          {PORTFOLIO_DATA.skills.map((group, idx) => (
            <div key={idx} className="bg-terminal-sidebar/60 border border-white/5 p-6 rounded-lg hover:border-primary/20 transition-colors">
              <div className="text-[10px] text-primary mb-4 font-bold tracking-widest uppercase">{group.name.replace('// ', '')}</div>
              <div className="flex flex-wrap gap-2">
                {group.items.map((item, i) => (
                  <span 
                    key={i} 
                    className="flex items-center gap-2 px-3 py-1.5 text-[11px] bg-white/[0.03] border border-white/10 text-gray-300 rounded-md hover:text-primary hover:border-primary/40 transition-all cursor-default group"
                  >
      ...
      <section className="max-w-4xl pb-20" id="contacts">
        <div className="flex items-center gap-4 mb-8">
          <span className="text-primary font-bold text-2xl">#</span>
          <h2 className="text-2xl font-bold text-white uppercase tracking-wider">Contact</h2>
          <div className="h-px bg-white/10 flex-1"></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-6">
            ...
            <a 
              className="flex items-center gap-4 text-gray-400 hover:text-primary transition-all group p-4 border border-white/5 hover:bg-white/5 rounded-lg" 
            ...
            <div className="space-y-8">
              {PORTFOLIO_DATA.contact.references.map((ref, idx) => (
                <div key={idx} className="relative pl-6 border-l-2 border-primary/20 hover:border-primary transition-colors">
                  <div className="text-sm font-bold text-white font-mono">{ref.name}</div>
